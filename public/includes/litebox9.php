...
#optinpoint .formbox > div:first-of-type{
  width: 65%;
  float: left;
}
#optinpoint .formbox > div:first-of-type + div{
  width: 35%;
  float: left;
  text-align: center;
}
#optinpoint .formbox input[type="text"]{
...
foreach ($form['fields'] as $f) {
  ...
  if($f['icon'] == 'idef'){
    ...
    $fi = $f['icon'];
  if($fi)
    echo '#optinpoint .oppo-ficon [oppofield="'.$f['id'].'"] ~ .inputicon {background: '.$this->getIcon($fi,15,$col).' no-repeat center}';
}
?>
...
.optinpoint-overlay-bg.signalshow .optinpoint-subs-button::after,
.optinpoint-overlay-bg.signalshow .optinpoint-subs-button::before{
  display: none;
}
...
'type' => 2,
'sig' => $this->getSpin('8','optinpoint-main',isset($theme["spinner_c"])?$theme["spinner_c"]:'#000')
);
