 ...
 *
 */
class OptinPoint_Lite_Assistant {
	/**
	 * @since   1.0.0
	 *
	 * @var      string
	 ...
	 * @var      string
	 */
	protected $plugin_slug = 'optinpoint-lite';
	...
	private function __construct() {
		...
		$this->orig_set = get_option('optinpoint_options');
		...
		add_action( 'widgets_init', create_function( '', 'register_widget("OptinPoint_Lite_Assistant_Widget");' ) );
		...
		add_action('wp_ajax_nopriv_optinpoint_lite_get_opt_sup',  array( $this, 'get_opt' ));

		add_action('wp_ajax_optinpoint_lite_add_email_ajax',  array( $this, 'subscribe' ));
		add_action('wp_ajax_nopriv_optinpoint_lite_add_email_ajax',  array( $this, 'subscribe' ));
	...
	public function load_plugin_textdomain() {
		...
		$this->settings = get_option('optinpoint_options');
	...
	public function loader(){
		if(!is_single() && !is_page() && !is_404()){
			...
			$cmeta = isset( $values['optinpoint_meta'] ) ? json_decode($values['optinpoint_meta'][0],true) : array();
			$cat = get_the_category();
			$option_name = 'optinpoint_meta_cat_' . (isset($cat[0])?$cat[0]->term_id:'');
	...
	public function integrateWithVC() {
        ...
        if(isset($this->orig_set["addon_scode"])){
	        vc_map( array(
	            "name" => __("OptinPoint", 'vc_extend'),
	            "description" => __("Subscription Box", 'vc_extend'),
	            "base" => "optinpoint",
	            "class" => "",
	            "icon" => plugins_url('assets/icon/optinpoint.svg', __FILE__),
	...
	public function enqueue_font(){
		global $oppo_font;
		if(empty($oppo_font))return false;
		$goo_fonts = array_diff(array_unique($oppo_font), $this->webfont());
		if(!empty($goo_fonts)){
	...
	public function enqueue_scripts() {
			wp_register_script( $this->plugin_slug . '-plugin-script',  OPPOINT_PLUGIN_URL. 'public/assets/js/public.js', array( 'jquery' ), self::VERSION );
	}
	/**
	...
	switch ($f['type']) {
		...
		case 'text':
			return '<input type="text"'
				...
				.' oppofield="'.$f['id'].'"'
				.' oppoformat="'.$f['format'].'"'
				.' opporeq="'.(((isset($f['req']) && $f['req']) || (isset($f['foreq']) && $f['foreq']))?'true':'false').'" required'
				.(($f['format'] == 'date')? ' oppomask="99/99/9999" oppomph="_"': '')
				.'/>'
		...
		case 'checkbox':
			$t = '';
			if(isset($f['extras'])){
				foreach ($f['extras'] as  $k =>$v) {
					$t.= '<label class="optinpoint-item">'
						.'<input type="checkbox"'
						.' name="oppo['.$f['id'].'][]"'
						.' oppofield="'.$f['id'].'"'
						.' value="'.$of[$k]['name'].'" oppotype="checkbox"'
						.' opporeq="'.(((isset($f['req']) && $f['req']) || (isset($f['foreq']) && $f['foreq']))?'true':'false').'"'
						.'>'
						.'<span>'.(isset($v['label'])?$v['label']:$v['name']).'</span>'
					.'</label>';
				}
			}
			return '<div class="optinpoint-itemh">'.$f['label'].'</div>'
				.'<div class="optinpoint-itemb">'
				.$t
				.'</div>';
		break;
		...
		case 'select':
			...
			if(isset($f['extras'])){
				foreach ($f['extras'] as $k =>$v) {
					$t.= '<option value="'.$of[$k]['name'].'">'.(isset($v['label'])?$v['label']:$v['name']).'</option>';
				}
			}
		...
		break;
		case 'multiselect':
			$t = '<option value="" disabled>'.$f['label'].'</option>';
			if(isset($f['extras'])){
				foreach ($f['extras'] as $k =>$v) {
					$t.= '<option value="'.$of[$k]['name'].'">'.(isset($v['label'])?$v['label']:$v['name']).'</option>';
				}
		...
		break;
		case 'textarea':
			return '<textarea placeholder="'.$f['label'].'"'
				.' name="oppo['.$f['id'].']"'
				.' oppotype="select"'
				.' oppofield="'.$f['id'].'"'
				.' opporeq="'.(((isset($f['req']) && $f['req']) || (isset($f['foreq']) && $f['foreq']))?'true':'false').'"'
	...
	foreach ($fields as $k =>$f) {
		...
		switch ($f['type']) {			
			case 'text':
				if($f['id']=='email'){
					if($set['type']==1)
						echo '<div class="optinpoint-field optinpoint-text'.(($f['icon']!='inone' && ($set['icon'] || (!$set['icon'] && $f['icon']!='idef')))?' oppo-ficon':'').'">'
						  .$this->fieldset($f)
						  .'<div class="oppoinfierr" oppoerr="email"></div>'
						.'</div>';
					else if ($set['type']==2){
						echo '<div class="formbox optinpoint-field">'
							.'<div class="optinpoint-text'.(($f['icon']!='inone' && ($set['icon'] || (!$set['icon'] && $f['icon']!='idef')))?' oppo-ficon':'').'">'
								.$this->fieldset($f)
								.'<div class="oppoinfierr" oppoerr="email"></div>'
							.'</div>'
							.'<div class="optinpoint-subsc"><div class="optinpoint-subs-button'.($set['bui'] ? ' subsicon':'').'"></div>'
							.'<div class="optinpoint-signal">'.$set['sig'].'</div>'
							.'</div><div style="clear:both"></div></div>';
					}
					else if ($set['type']==3){
						echo '<div class="formbox optinpoint-field optinpoint-text'
								.(($f['icon']!='inone' && ($set['icon'] || (!$set['icon'] && $f['icon']!='idef')))?' oppo-ficon':'')
								.(($top == $k)? ' oppotop':'')
								.(($bot == $k)? ' oppobot':'')
								.(((isset($f['req']) && $f['req']) || (isset($f['foreq']) && $f['foreq']))?' oppofreq':'')
								.'">'
							.$this->fieldset($f)
							.'<div class="oppoinfierr" oppoerr="email"></div>'
							.'<div class="optinpoint-subs-button"></div>'
							.'<div class="optinpoint-signal">'.$set['spin'].'</div>'
							.'</div>';
					}
					else if ($set['type']==4){
						echo '<div class="optinpoint-field optinpoint-text'.(($f['icon']!='inone' && ($set['icon'] || (!$set['icon'] && $f['icon']!='idef')))?' oppo-ficon':'').'">'
					...
					if($set['type']==1 || $set['type']==2){
						echo '<div class="optinpoint-field optinpoint-text'.((($f['icon']!='idef' & $f['icon']!='inone') || ($set['icon'] && $f['icon']=='idef' && ($f['id']=='name' || $f['id']=='fname' || $f['id']=='lname')))?' oppo-ficon':'').'">'
							.$this->fieldset($f)
							.'<div class="oppoinfierr" oppoerr="'.$f['id'].'"></div>'
							.'</div>';
					}
					else if ($set['type']==3){
						echo '<div class="optinpoint-field optinpoint-text'
								.(($f['icon']!='inone' && ($set['icon'] || (!$set['icon'] && $f['icon']!='idef')))?' oppo-ficon':'')
								.(($top == $k)? ' oppotop':'')
								.(($bot == $k)? ' oppobot':'')
								.(((isset($f['req']) && $f['req']) || (isset($f['foreq']) && $f['foreq']))?' oppofreq':'')
			...
			case 'checkbox':
				if($set['type']==1 || $set['type']==2 || $set['type']==4){
					echo '<div class="optinpoint-field optinpoint-check oppomul">'
						.$this->fieldset($f,$ofield[$k]['extras'])
					  .'<div class="oppoinfierr" oppoerr="'.$f['id'].'"></div>'
					.'</div>';
				}
				else if ($set['type']==3) {
			...
			case 'radio':
				if($set['type']==1 || $set['type']==2 || $set['type']==4){
					echo '<div class="optinpoint-field optinpoint-radio oppomul">'
						.$this->fieldset($f,$ofield[$k]['extras'])
					  .'<div class="oppoinfierr" oppoerr="'.$f['id'].'"></div>'
					.'</div>';
				}
			...
			case 'select':
				if($set['type']==1 || $set['type']==2 || $set['type']==4){
					echo '<div class="optinpoint-field optinpoint-drop">'
						.$this->fieldset($f,$ofield[$k]['extras'])
					  .'<div class="oppoinfierr" oppoerr="'.$f['id'].'"></div>'
					.'</div>';
				}
			...
			case 'multiselect':
				if($set['type']==1 || $set['type']==2 || $set['type']==4){
					echo '<div class="optinpoint-field optinpoint-multidrop">'
						.$this->fieldset($f,$ofield[$k]['extras'])
					  .'<div class="oppoinfierr" oppoerr="'.$f['id'].'"></div>'
					.'</div>';
				}
			...
			case 'textarea':
				if($set['type']==1 || $set['type']==2 || $set['type']==4){
					echo '<div class="optinpoint-field optinpoint-textarea">'
						.$this->fieldset($f)
					  .'<div class="oppoinfierr" oppoerr="'.$f['id'].'"></div>'
					.'</div>';
				}
...
public function social_connect(){
	...
	if(isset($this->settings['ms_api'])){
		...
		WL.init({
		    ...
		    redirect_uri: "<?php echo OPPOINT_PLUGIN_URL.'assets/ms-oauth.php';?>"
		});
		</script>
		<?php
	}
...
function get_weather($loc,$api) {
	...
	public function subscribe($a){
		...
		if(!isset($settings['mailserv']['id'])) die('-1');
		$form = $this->getformbyid($_POST['oppoform']);
		$serv = new OptinPoint_Lite_Service($settings['mailserv']);
		$_POST['oppo'] = array_filter($_POST['oppo'],array($this , 'myFilter'));
		$r = $serv->subscribe($form,$_POST['oppo']);
		if($a == 1)return;
		echo $r;
		die('');
	}
...
function register_shortcodes(){
	if(isset($this->settings["addon_scode"]))
   		add_shortcode('optinpoint', array($this,'addon_scode'));
}
	/**
...
function addon_scode($atts, $content = null) {
	...
	private static function single_activate() {
		...
		add_option('optinpoint_options',$json);
		$ab = array("status" => 0);
		update_option('optinpoint_ver',self::VERSION);
		/*if (function_exists('curl_init') && function_exists('curl_setopt')){
			$curl = curl_init();
			curl_setopt_array($curl, array(
			    CURLOPT_RETURNTRANSFER => 1,
			    CURLOPT_URL => 'http://voltroid.com/optinpoint/api.php',
	...
	public function commentfield( $comment_field ) {
		if(isset($this->settings['usyn_com']) && isset($this->settings['usyn_comp']) && $this->settings['usyn_comp']=='1')
	    	return $comment_field.'<label><input type="checkbox" name="optinpoint" value="1">'.(isset($this->settings['usyn_compt'])?$this->settings['usyn_compt']:__( 'Subscribe to Newsletters', 'optinpoint' )).'</label>';
	    return $comment_field;
	}
	public function putname($user,$fname, $lname=''){
		switch ($this->settings['mailserv']['id']) {
			case 'aw':
			case 'cm':
			case 'gr':
	...
	public function commentpost($comment_ID){
		if(isset($this->settings['usyn_com']) && isset($this->settings['usyn_comp'])){
			if(($this->settings['usyn_comp']=='1' && isset ( $_POST['optinpoint'] )) || ($this->settings['usyn_comp']=='0')){
				if(is_user_logged_in()){
					global $current_user;
		  			...
		  			$_POST['oppo'] = $this->putname($user, $current_user->user_firstname, $current_user->user_lastname);
				}
				else{
		  			$user=array(
		  				'email' => (is_email($_POST['email']))?$_POST['email']:''
		  			);
		  			$_POST['oppo'] = $this->putname($user, $_POST['author']);
				}
				$_POST['oppoform'] = $this->settings['usyn_coml'];
				$this->subscribe(1);
			}
		}
	}
	...
	public function regfield(){
		if(isset($this->settings['usyn_reg']) && isset($this->settings['usyn_regp']) && $this->settings['usyn_regp']=='1')
	    	echo '<label><input type="checkbox" name="optinpoint" value="1">'.(isset($this->settings['usyn_regpt'])?$this->settings['usyn_regpt']:__( 'Subscribe to Newsletters', 'optinpoint' )).'</label>';
	}
	/**
	...
	public function regpost(){
		if(isset($this->settings['usyn_reg']) && isset($this->settings['usyn_regp'])){
			...
			if(($this->settings['usyn_regp']=='1' && isset ( $_POST['optinpoint'] )) || ($this->settings['usyn_regp']=='0' && in_array($roles[0], $this->settings['usync_role']))){
	  			$_POST['oppo']=array(
	  				'email' => (is_email($_POST['user_email']))?$_POST['user_email']:''
	  			);
				$_POST['oppoform'] = $this->settings['usyn_regl'];
				$this->subscribe(1);
			}
		}
	}
