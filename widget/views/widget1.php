...
#<?php echo $oppow_id;?> .optinpoint-leftpane{
width: 100%;
...
foreach ($form['fields'] as $f) {
  if($f['icon'] != 'idef' && $f['icon'] != 'inone')
    echo '#'.$oppow_id.' .oppo-ficon [oppofield="'.$f['id'].'"] ~ .inputicon {background: '.$plugin->getIcon($f['icon'],30,$col).' no-repeat center}';
}
?>
...
#<?php echo $oppow_id;?> .optinpoint-social .optinpoint-soc:hover{
...
?>
  <div class="optinpoint-subs-button<?php echo (isset($theme['button_i']) && $theme['button_i'] != 'inone' && $theme['button_i'] != 'idef')? ' subsicon' : '';?>"></div>
            ...
            echo $plugin->getSpin('4',$oppow_id,isset($theme["spinner_c"])?$theme["spinner_c"]:'#000');?></div></div>
		</form>
